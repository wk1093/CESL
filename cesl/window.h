...
// this is just for window management, no graphics or GUI, that is done in gui.h
// and gfx.h, so opengl functions are not needed here
...
struct Window {
    ...
    int height;
    char *title;
    struct _WindowVal old; // if someone manually changes the Window.width, we
                           // can realize it, and update the window
};

struct WindowSettings {
    // commonly used window hints
    bool resizable;
    bool visible;
    bool decorated;
    bool focused;
...
ceslf(struct WindowSettings *defaultWindowSettings(), {
    struct WindowSettings *settings =
        (struct WindowSettings *)malloc(sizeof(struct WindowSettings));
    settings->resizable = true;
    settings->visible = true;
    settings->decorated = true;
    settings->focused = true;
    settings->autoIconify = true;
    settings->floating = false;
    settings->maximized = false;
    return settings;
});

ceslf(void initWindow(), {
    if (!glfwInit()) {
...
ceslf(struct Window *newWindow(char *title, int width, int height,
                               struct WindowSettings *settings),
      {
          ...
          if (settings == NULL) {
              printf("Using default window settings\n");
              settings = defaultWindowSettings();
          } else {
              printf("Using custom window settings\n");
          }
          ...
          win->old.width = win->width;
          win->old.height = win->height;
          win->old.title = win->title;
          return win;
      });

ceslf(bool updateWindow(struct Window *win), {
    if (win->old.width != win->width || win->old.height != win->height) {
        glfwSetWindowSize(win->window, win->width, win->height);
        win->old.width = win->width;
        win->old.height = win->height;
    }
    if (strcmp(win->old.title, win->title) != 0) {
        glfwSetWindowTitle(win->window, win->title);
        win->old.title = win->title;
    }
...
ceslf(void flipWindow(struct Window *win), { glfwSwapBuffers(win->window); });
